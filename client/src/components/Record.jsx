...
export default function Record() {
  ...
  const handleAddSlot = () => {
    if (selectedSlot.trim()) {
      updateForm({
        stylistAvailabilities: [...form.stylistAvailabilities, selectedSlot],
      });
      setSelectedSlot("");
    } else {
      alert("Please select a valid time slot.");
  ...
  const handleAddHairstyle = () => {
    ...
    if (name && time && price) {
      ...
      setHairstyleInput({ name: "", time: "", price: "" });
    } else {
      alert("Please fill in all hairstyle fields.");
    }
  };
  ...
  const handleLoginNavigate = () => {
    navigate("/login"); // Route to the login page
  };
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // Send user data to your backend for storing additional profile details
      const response = await fetch(
        `https://hairbnbbe-9f629b6e0127.herokuapp.com/record${params.id ? `/${params.id}` : ""}`,
        {
          method: params.id ? "PATCH" : "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...form,
            uid: firebaseUser.uid, // Link Firebase UID to user in MongoDB
          }),
        }
      );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white border-2 border-gray-100 rounded-lg shadow-lg mt-2">
      ...
      <form onSubmit={onSubmit}>
        ...
        {/* Additional Fields for Stylist/Customer */}
        {form.level === "Stylist" && (
                    <div className="mt-10">
                    <h3 className="text-xl font-semibold text-gray-700 mb-4">
                      Stylist Details
        ...
        <div className="mt-10">
          <button
            type="submit"
            className="w-full md:w-auto px-6 py-3 bg-pink-500 text-white rounded-md shadow-sm hover:bg-pink-600 transition"
          >
            Save Profile
          </button>
        </div>
      ...
      <div className="mt-4">
        <button
          type="button"
          onClick={handleLoginNavigate}
          className="w-full md:w-auto px-6 py-3 bg-blue-500 text-white rounded-md shadow-sm hover:bg-blue-600 transition"
        >
          Login
        </button>
      </div>
